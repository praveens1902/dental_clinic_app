...
import { MedicalHistoryTimelineItem } from '../types'
import { Clock, Activity } from 'lucide-react'
import { EmptyState } from '@/components/ui/EmptyState'
import { Skeleton } from '@/components/ui/Skeleton'

interface MedicalTimelineViewerProps {
  timeline: MedicalHistoryTimelineItem[]
  isLoading?: boolean
}

export const MedicalTimelineViewer: React.FC<MedicalTimelineViewerProps> = ({
  timeline,
  isLoading = false,
}) => {
  if (isLoading) {
  ...
  if (timeline.length === 0) {
    return (
      <EmptyState
        ...
        description="Medical history timeline events will appear here once records are created or updated."
        icon={Activity}
      />
    )
  }
