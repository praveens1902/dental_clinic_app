...
interface ClinicalTimelineViewerProps {
  timeline: ClinicalTimelineItem[]
  isLoading?: boolean
}

export const ClinicalTimelineViewer: React.FC<ClinicalTimelineViewerProps> = ({
  timeline,
  isLoading = false,
}) => {
  if (isLoading) {
  ...
  if (timeline.length === 0) {
    return (
      <EmptyState
        ...
        description="Clinical timeline events will appear here once treatments are logged or plans formulated."
        icon={History}
      />
    )
  }
    ...
    <div className="relative pl-4 border-l border-border/80 space-y-5 pt-1 ml-2">
      {timeline.map((item) => (
        <div key={item.id} className="relative text-left">
          {/* Connector Bullet */}
          <div className="absolute -left-[21.5px] top-1 w-2.5 h-2.5 rounded-full bg-primary ring-4 ring-white animate-pulse" />
...
export default ClinicalTimelineViewer
